...
%--------------------------------------------------------------------------
Name='Context7_AllOutOne_TrainWithCntk_LabelType1ValidationType1LandmarkType2_NonLandmarkTag';
Thr=-1000;
Thr2=0.1,
...
GoldTagsLandmarkname2(395,size(TestOut,1))=0;

for i=1:size(TestOut,1)
    if GoldTags.total.flag(i)=='n'
        GoldTagsLandmarkname(i)=395;
        GoldTagsLandmarkname2(395,i)=1;
    ...
    elseif  GoldTags.total.flag(i)=='b'
        for k=1:364
            if GoldTags.event.part1(i)==LandmarkType2_Events(k,1) && GoldTags.event.part2(i)==LandmarkType2_Events(k,2)
...
TestTagsLandmarkname2(395,size(TestOut,1))=0;
for i=1:size(TestOut,1)
    if  TestTags.total.flag(i)=='b'
%         for k=1:364
%             if TestTags.event.indexpart1(i)==LandmarkType2_Events(k,1) && TestTags.event.indexpart2(i)==LandmarkType2_Events(k,2)
%                 k
%                 TestTagsLandmarkname(i)=k+30;
%                 TestTagsLandmarkname2(k+30,i)=1;
%             else
%                 TestTagsLandmarkname(i)=395;
%                 TestTagsLandmarkname2(395,i)=1;
%             end
%         end
        BorderFlag=0;
        for k=1:364
...
for i=31:394
    LL{i}=strcat(SB(LandmarkType2_Events(i-30,1)), SB(LandmarkType2_Events(i-30,2)));
end
LL{395}={'NN'};
